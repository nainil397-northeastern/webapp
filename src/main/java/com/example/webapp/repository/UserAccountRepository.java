...
public interface UserAccountRepository extends JpaRepository<UserAccountModel,Integer> {
    ...
    @Modifying
    @Transactional
    @Query(value = "INSERT INTO mywebapp (first_name, last_name, username, psswrd, account_created, account_updated, authority, enabled) VALUES ( ?1 , ?2 , ?3 , ?4 , ?5, ?6, 'USER', 1)", nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE mywebapp SET first_name = ?1, last_name = ?2, username = ?3, psswrd = ?4, account_updated = ?5 WHERE id = ?6", nativeQuery = true)
